...
Publish/Subscribe server to exchange messages between Elixir and [`elixirkit::PubSub`].

PubSub is designed to be started alongside [`elixirkit::PubSub`] from a native app. Otherwise,
broadcasts are dropped with a warning so development can proceed without Rust.
...
```elixir
@spec broadcast(topic :: String.t(), message :: String.t()) :: :ok
```
...
## Options
  ...
  * `:on_exit` - a zero-arity function called when PubSub exits
    (e.g. when the Rust side disconnects).

    > #### Handling exit {: .warning}
    >
    > It is recommended to set `on_exit: fn -> System.stop() end` to cleanly
    > shutdown the VM when the Rust side exits abruptly.
    >
    > Alternatively, consider monitoring the `ElixirKit.PubSub` process and
    > respond accordingly.
    >
    > Finally, you may instead set `{ElixirKit.PubSub, significant: true}` in
    > your supervision tree and set your supervisor to

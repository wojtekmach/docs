...
Elixir/Rust interoperability toolkit.

## Features

- Spawn Elixir process from Rust and exchange messages over [PubSub].
- Rust **ports** — external programs that run in a separate OS process; crashes are isolated and won't affect the VM.
- Rust **NIFs** — native functions that run in the same memory space as the VM, with low overhead and best performance.
- Precompile NIFs and ports with `mix elixirkit.precompile` — cross-compile for multiple targets so end users don't need Rust installed.
